...
You'll need the following tools installed before you can build Meteroic:
...
After installation, restart your shell or run `source ~/.cargo/env` to make `cargo` available. Meteroic builds against stable Rust.
...
**macOS:** Meteroic is macOS-only. It uses NSPanel and Core Graphics APIs that are not available on other platforms.
...
1. **Fork and clone the repository**
   ...
   ```bash
   git clone https://github.com/quiet-node/Meteroic.git
   cd Meteroic
   ```
...
3. **Set up your AI backend** (choose one):
   ...
   **Option B: Local Ollama**
   ...
   Meteroic connects to `http://127.0.0.1:11434` by default.
...
New to the codebase? Look for issues tagged [`good first issue`](https://github.com/quiet-node/Meteroic/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22) on GitHub. These are scoped to be approachable without deep knowledge of the full system.
