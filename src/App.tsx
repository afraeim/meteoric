...
import { ProviderSetupStep } from './view/onboarding/ProviderSetupStep';
...
const DEFAULT_MODEL_FALLBACK = 'gpt-4.1-mini';

const OVERLAY_VISIBILITY_EVENT = 'meteoric://visibility';
const ONBOARDING_EVENT = 'meteoric://onboarding';
const LEGACY_OVERLAY_VISIBILITY_EVENT = 'meteoric://visibility';
const LEGACY_ONBOARDING_EVENT = 'meteoric://onboarding';
...
/**
 * Main application orchestrator for Meteoric.
 *
...
function App() {
  ...
  const [providerSetupStatus, setProviderSetupStatus] =
    useState<ProviderSetupStatus | null>(null);
   ...
   * (excluding Meteoric's own windows) and returns the saved file path.
  ...
  useEffect(() => {
    ...
    void invoke<ProviderSetupStatus>('get_provider_setup_status').then(
      (status) => {
        if (status && typeof status.needsSetup === 'boolean') {
  ...
  const refreshProviderSetupStatus = useCallback(async () => {
    const status = await invoke<ProviderSetupStatus>('get_provider_setup_status');
    setProviderSetupStatus(status);
    const nextModelConfig = await invoke<{ active: string; all: string[] }>(
      'get_model_config',
    );
    setModelConfig(nextModelConfig);
  }, []);

  /** Keep the latest callbacks available to long-lived event listeners. */
  const replayEntranceAnimationRef = useRef(replayEntranceAnimation);
  const requestHideOverlayRef = useRef(requestHideOverlay);

  useEffect(() => {
    replayEntranceAnimationRef.current = replayEntranceAnimation;
    requestHideOverlayRef.current = requestHideOverlay;
  ...
  useEffect(() => {
    ...
    const onVisibilityEvent = ({
      ...
      payload: OverlayVisibilityPayload;
    }) => {
      if (payload.state === 'show') {
        replayEntranceAnimationRef.current(
      ...
      requestHideOverlayRef.current();
    };

    const onOnboardingEvent = ({
      payload,
    }: {
      payload: { stage: OnboardingStage };
    }) => {
    ...
    const attachListeners = async () => {
      unlisteners.push(
        await listen<OverlayVisibilityPayload>(
      ...
      unlisteners.push(
        await listen<OverlayVisibilityPayload>(
          LEGACY_OVERLAY_VISIBILITY_EVENT,
          onVisibilityEvent,
        ),
      );
      unlisteners.push(
        await listen<{ stage: OnboardingStage }>(
          ONBOARDING_EVENT,
          onOnboardingEvent,
        ),
      );
  ...
  if (providerSetupStatus?.needsSetup) {
    return (
      <ProviderSetupStep
        initialProvider={providerSetupStatus.activeProvider}
        onComplete={() => {
