...
async function showOverlay(selectedText: string | null = null) {
  await act(async () => {
    emitTauriEvent('meteoric://visibility', {
      state: 'show',
...
describe('App', () => {
  ...
  it('grows upward when near bottom screen edge', async () => {
    ...
    const textarea = screen.getByPlaceholderText('Ask Meteoric anything...');
    await act(async () => {
  ...
  it('shows overlay on visibility show event', async () => {
    ...
    expect(
      screen.getByPlaceholderText('Ask Meteoric anything...'),
    ).toBeInTheDocument();
  });
  ...
  it('enters hiding state on hide-request visibility event', async () => {
    ...
    await act(async () => {
      emitTauriEvent('meteoric://visibility', { state: 'hide-request' });
    });
    ...
    expect(
      screen.queryByPlaceholderText('Ask Meteoric anything...'),
    ).toBeNull();
  });
  ...
  describe('Onboarding', () => {
    it('shows onboarding screen when meteoric://onboarding event fires', async () => {
      enableChannelCaptureWithResponses({
      ...
      await act(async () => {
        emitTauriEvent('meteoric://onboarding', { stage: 'permissions' });
      });

      expect(screen.getByText("Let's get Meteoric set up")).toBeInTheDocument();
    });
    ...
    it('does not show onboarding on normal visibility event', async () => {
      ...
      expect(screen.queryByText("Let's get Meteoric set up")).toBeNull();
    });
    ...
    it('dismisses onboarding and shows ask bar when onComplete is called', async () => {
      ...
      await act(async () => {
        emitTauriEvent('meteoric://onboarding', { stage: 'intro' });
      });
