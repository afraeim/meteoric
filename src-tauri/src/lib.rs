/*!
 * Meteroic Core Library
 *
 * Application bootstrap for the Meteroic desktop agent. Configures the macOS
...
// MeteroicPanel — custom NSPanel subclass for the overlay.
...
const OVERLAY_VISIBILITY_EVENT: &str = "meteoric://visibility";
...
#[cfg(target_os = "macos")]
const ONBOARDING_EVENT: &str = "meteoric://onboarding";
...
fn show_overlay(app_handle: &tauri::AppHandle, ctx: crate::context::ActivationContext) {
    ...
    match app_handle.get_webview_panel("main") {
        ...
        Err(e) => {
            eprintln!("Meteroic: [show_overlay] get_webview_panel FAILED: {e:?}");
...
/// visible, and emits `meteoric://onboarding` so the frontend switches to
/// `OnboardingView`.
///
...
pub fn run() {
    // Load .env file so METEORIC_SYSTEM_PROMPT and future backend env vars
    ...
    builder
        .setup(|app| {
            ...
            // Linux note: some environments (including NixOS shells) may not
            // provide libayatana-appindicator/libappindicator at runtime,
            // causing tray initialization to panic before the window opens.
            // We skip tray creation on Linux so desktop dev mode still runs.
            #[cfg(not(target_os = "linux"))]
            {
                let show_item =
                    MenuItem::with_id(app, "show", "Open Meteoric", true, None::<&str>)?;
            ...
            #[cfg(target_os = "macos")]
            {
                ...
                if permissions::is_accessibility_granted() {
                    activator.start(move || {
                        ...
                        // simulating Cmd+C against Meteroic's own WebView would produce
                        // a macOS alert sound.
            ...
            let provider_cfg = commands::load_provider_config_with_db(Some(&db_conn));
            let model_cfg =
                commands::load_model_config_with_provider(Some(&db_conn), provider_cfg.active);

            app.manage(history::Database(std::sync::Mutex::new(db_conn)));
            app.manage(commands::ProviderConfigState(std::sync::Mutex::new(
                provider_cfg,
            )));
            app.manage(commands::ModelConfigState(std::sync::Mutex::new(model_cfg)));
...
mod tests {
    ...
    fn overlay_visibility_event_constant_matches() {
        assert_eq!(OVERLAY_VISIBILITY_EVENT, "meteoric://visibility");
    ...
    fn onboarding_event_constant_matches() {
        assert_eq!(ONBOARDING_EVENT, "meteoric://onboarding");
    }
