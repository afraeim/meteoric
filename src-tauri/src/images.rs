...
mod tests {
    ...
    fn temp_dir() -> PathBuf {
    let dir = std::env::temp_dir().join(format!("Meteroic-test-{}", uuid::Uuid::new_v4()));
    ...
    fn remove_image_idempotent_on_missing_file() {
        let base = temp_dir();
    let result = remove_image(&base, "/tmp/nonexistent-Meteroic-image.jpg");
        assert!(result.is_ok());
        fs::remove_dir_all(&base).unwrap();
    }
    ...
    let result = encode_images_as_base64(&["/tmp/nonexistent-Meteroic.jpg".to_string()]);
        assert!(result.is_err());
    }

    #[test]
    fn images_root_resolves_correctly() {
    let base = Path::new("/tmp/Meteroic-test");
    assert_eq!(images_root(base), PathBuf::from("/tmp/Meteroic-test/images"));
    }
