 ...
 *    CoreGraphics `CGWindowListCreateImageFromArray`, excluding Meteroic's own
...
/// Returns a unique `/tmp/<uuid>-Meteroic.png` path for a single screenshot capture.
/// A new UUID is generated on every call, preventing collisions.
pub fn temp_screenshot_path() -> PathBuf {
    PathBuf::from(format!("/tmp/{}-Meteroic.png", uuid::Uuid::new_v4()))
}
...
///
/// Captures all on-screen content below Meteroic's own window in the Z-order,
/// effectively excluding Meteroic from the screenshot without hiding the window.
...
fn capture_full_screen_raw() -> Result<(u32, u32, Vec<u8>), String> {
    ...
    unsafe {
        ...
        if !CGPreflightScreenCaptureAccess() {
            ...
            return Err(
                "Screen Recording permission was just granted but needs a restart to \
                 activate. Please quit and relaunch Meteroic, then try /screen again."
                    .to_string(),
            );
        }
        ...
        if window_info_list.is_null() {
          ...
          return Err("Screen Recording permission check failed unexpectedly. \
              Try restarting Meteroic."
                .to_string());
        }

    // Find Meteroic's own topmost window ID so we can capture everything
    ...
    // kCGWindowListOptionOnScreenBelowWindow already excludes Meteroic itself by
    ...
    // on-screen windows. Meteroic is transparent so its presence in the list does
...
/// Tauri command: silently captures the full screen (excluding Meteroic's own
...
mod tests {
    ...
    fn temp_screenshot_path_is_in_tmp_and_ends_with_png() {
        ...
        assert!(
            s.ends_with("-Meteroic.png"),
            "expected -Meteroic.png suffix, got: {s}"
        );
    }
