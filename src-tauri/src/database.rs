 ...
 *
 * Stores conversations and messages in `~/.Meteroic/Meteroic.db` using rusqlite
...
/// Opens (or creates) the SQLite database at `<app_data_dir>/Meteroic.db` and
/// runs migrations. If an existing database is found at the legacy location
/// (`~/.Meteroic/Meteroic.db`), it is moved to the new location automatically.
///
/// # Errors
///
...
pub fn open_database(app_data_dir: &std::path::Path) -> SqlResult<Connection> {
    ...
    let db_path = app_data_dir.join("Meteroic.db");

    // One-time migration: move database from the legacy ~/.Meteroic/ location.
...
/// Moves the database from `~/.Meteroic/Meteroic.db` to the Tauri app data
...
fn migrate_legacy_db(new_path: &std::path::Path) {
    ...
    Some(home) => home.join(".Meteroic").join("Meteroic.db"),
        None => return,
    };
...
mod tests {
    ...
    let new_path = tmp.join("Meteroic.db");
