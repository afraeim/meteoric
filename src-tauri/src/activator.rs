//! Unified activation and visibility management for the meteoric overlay.
//!
    ...
    loop {
        ...
        match try_initialize_tap(&is_active, &on_activation) {
            ...
            TapExitReason::TapDied => {
                ...
                eprintln!("Meteoric: [activator] tap died — reinstalling");
            ...
            TapExitReason::CreationFailed => {
                ...
                if permission_failures >= MAX_PERMISSION_ATTEMPTS {
                    eprintln!(
                        "meteoric: [error] activation listener failed after \
                         maximum retries; check system permissions."
                    );
                    return;
                }
                eprintln!(
                    "meteoric: [activator] tap creation failed \
    ...
    F: Fn() + Send + Sync + 'static,
{
    ...
    // permission, which meteoric already holds.
    let tap_result = CGEventTap::new(
        ...
        move |_proxy, event_type, event: &CGEvent| -> CallbackResult {
            ...
            if matches!(
                ...
                eprintln!(
                    "meteoric: [activator] event tap disabled by macOS \
    ...
    match tap_result {
        Ok(tap) => {
            eprintln!("Meteoric: [activator] event tap created (HID level) — listening for double-tap Control");
            unsafe {
                let loop_source = tap
                    .mach_port()
            ...
            eprintln!("meteoric: [activator] event tap run loop exited");
        ...
        Err(()) => {
            eprintln!(
                "meteoric: [activator] event tap creation FAILED; check Accessibility permission"
            );
            TapExitReason::CreationFailed
        }
    }
