...
Meteoric uses environment variables for runtime configuration. Vite loads these from `.env` files at build/dev time and exposes variables prefixed with `VITE_` to the frontend via `import.meta.env`.
...
| :--- | :--- | :--- |
| `METEORIC_SYSTEM_PROMPT` | Custom system prompt for all conversations. If unset or empty, the built-in default is used. | Built-in secretary persona prompt (see `src-tauri/src/commands.rs`) |

### Model Configuration

Controls which Ollama model(s) Meteoric uses for inference.

| Variable | Description | Default |
| :--- | :--- | :--- |
| `METEORIC_SUPPORTED_AI_MODELS` | Comma-separated list of Ollama model names. The first entry is the active model used for all inference. Additional entries are available for future in-app model switching. | `gemma4:e2b` |

**Example:**

```bash
# Single model (default behavior)
METEORIC_SUPPORTED_AI_MODELS=gemma4:e2b

# Multiple models (first is active; others available for future UI picker)
METEORIC_SUPPORTED_AI_MODELS=gemma4:e2b,gemma4:e4b
```

Whitespace around each entry is trimmed. Empty entries are ignored. If the variable is unset or empty, Meteoric falls back to `gemma4:e2b`.
